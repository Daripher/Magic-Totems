...
public class TotemsConfiguredFeature {
	public static ConfiguredFeature<?, ?> create(RegistryObject<Block> blockObject, TagKey<Block> acceptableBlocksBelow) {
		var blockBelowPredicate = BlockPredicate.matchesTag(BlockPos.ZERO.below(), acceptableBlocksBelow);
		var blockAtPosPredicate = BlockPredicate.matchesBlocks(Blocks.AIR);
		var blockAbovePredicate = BlockPredicate.matchesBlocks(BlockPos.ZERO.above(), Blocks.AIR);
